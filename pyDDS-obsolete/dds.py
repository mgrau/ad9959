...
class DDS():
    ...
    def __init__(self, device=0):
        ...
        self.freqmult = 1
        self.set_freqmult(10)
        self.clock_freq = self.refclock_freq*self.freqmult
        
        # reset the DDS
        self.reset()

        # put communication in 3 wire mode
        # and select channel (channel 0 default)
        self.set_channels(0)
        ...
        #TODO:
        # figure out how to kill class gracefully
        # so the gpio and spi resources are properly
...
#        gpio.cleanup()
    ...
    def ioupdate(self,):
        ...
        gpio.output(IOUPDATE_PIN, 0)
        gpio.output(IOUPDATE_PIN, 1)
        gpio.output(IOUPDATE_PIN, 0)
    ...
    def init(self, freqmult=10, channels=0):
        ...
        """
        self.reset()
        # other things to initialize the state of the DDS how we want it to be
    ...
    def set_channels(self, channels, ioupdate=False):
        ...
        assert (type(channels) is int) or (type(channels) is list), \
        'Channels must be int or list'
    ...
    def set_refclock(self, frequency):
        ...
        Automatically updates class variable self.clock_freq with new refclock frequency.
        Must pass frequency in Hz (i.e 50MHz = 50e6 Hz)
        Prints out current values for self.refclock_freq, self.freqmult and self.clock_freq
        """
    ...
    def set_freqmult(self, freqmult, ioupdate=False):
        ...
        Automatically updates self.clock_freq with new freqmult.
        When changing the frequency multiplier the channel frequencies must be reset.
        
        Frequency multiplier must be between 4 and 20 or equal to 1 (turns off multiplier)
        Frequency multiplier must be chosen such that self.clock_freq lies between \
        100MHz and 500MHz.
    ...
    def set_frequency(self, channels, frequency, ioupdate=False):
        ...
        frequency must be passed in Hz.
    ...
    def get_frequency(self,):
        ...
        # Returns values saved in set_frequency
    ...
    def set_current(self, channels, divider, ioupdate=False):
        ...
        Current divider must be 1, 2, 4 or 8 corresponding to 1, 1/2, 1/4 and 1/8 scales respectively.
        channels can be a single int from 0, 1, 2 or 3 or a list of several of 0, 1, 2 or 3.
        Setting ioupdate=True will issue an ioupdate to write the settings into the DDS registers.
        """
        
        #Assert divider is in [1, 2, 4, 8]
    ...
    def set_amplitude(self, channels, scale_factor, ioupdate=False):
        ...
        """
        Sets amplitude scale factor to selected channels. If scale_factor == 1, 
        ASF is switched off. Otherwise the ASF is turned on.
        ...
        #Sets ASF and toggles the multiplier bit on ACR[12], 
        #without changing the rest of ACR register.
    ...
    def get_amplitude(self,):
        ...
        # Returns values saved in set_amplitude
    ...
    def set_phase(self, channels, phase, ioupdate=False):
        ...
        The phase must be given in degress and must lie between 0 and 359.987 deg.
        Setting ioupdate=True will issue an ioupdate to write the settings into the DDS registers.
    ...
    def get_phase(self,):
        ...
        print ('Latest phase set (i.e. currently in register):', phase)
         ...
         # Returns values saved in set_phase
        return self.phases
    ...
    def sweep_loop(self, channels, reps, interval):
        ...
        """
        Initiates a loop of reps PIN toggles with an interval
        between every toggle. channels indicates which channel PINS should be toggled.
        interval indicates the interval between every toggle in seconds.
        """
    ...
    def select_CHPINS(self, channels):
        assert (type(channels) is int) or (type(channels) is list),\
        'channels must be passed as int or list'
